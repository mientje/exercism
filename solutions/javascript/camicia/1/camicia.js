...
const addRound = (camicia) => {
	...
	for(let i = 0; i < rounds.length; i++) {
		...
		camicia["rounds"][camicia.rounds.length-1].push(rounds[i]);
	}
	return camicia.rounds
...
const compareRounds = (camicia) => {
	let loops = 0;
	if(camicia.rounds.length > 1) {
		const lastRound = camicia.rounds[camicia.rounds.length-1];
		for(let i = 0; i < camicia.rounds.length-1; i++) {
			loops = oneRound(camicia.rounds[i], lastRound);
...
const playHand = (camicia) => {
	...
	if(camicia[playerName].length === 0 ) {
		...
		camicia[camicia.playerName].push(...camicia['pile'].splice(0));		
		return camicia;
	}
	addToPile(camicia);
	camicia.payment = setPayment(camicia['pile'][camicia['pile'].length-1]);
...
const addToPile = (camicia) => {
	camicia['pile'].push(camicia[camicia.playerName].shift());								
...
const pay = (camicia) => {
	...
	while(camicia.payment > 0) {
		// betaler heeft geen kaarten meer : move gaat terug naar de andere 
		...
		const last = camicia['pile'][camicia['pile'].length-1];
		if(/\D/.test(last)) {
		...
		if(camicia.payment === 0) {
			// person who placed the last payment card collects pile when payment is done
			if(/\D/.test(last)) {
				camicia.payment = setPayment(last);
				return pay(camicia)
			}
