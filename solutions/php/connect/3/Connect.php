...
function trace($game, $traces, $player) : string{
    ...
    foreach($traces as $key => $trace)  {
        ...
        while(end($trace)[$playerCoord] !== $endTrace) {
            ...
            //to the left of XorO and if the trace stopped on a position bigger than O
            if($game[$tailLine][$tailPos-2]  === $player 
                && $tailPos > 0 ) {
                $added[] = [$tailLine, $tailPos-2];
            }
            // beneath XorO and if the trace stopped on a line that's above the bottom line
            if($game[$tailLine+1][$tailPos]  === $player 
                && $tailLine < $endTrace) {
                $added[] = [$tailLine+1, $tailPos];
            }
            // above XorO an if the trace stopped on a line that's beneath the top line
            if($game[$tailLine-1][$tailPos]  === $player 
                && $tailLine > 0) {
                $added[] = [$tailLine-1, $tailPos];
            }
            //to the right above XorO and the trace stopped on a line beneath the top line
            if($game[$tailLine-1][$tailPos+2]  === $player 
                && $tailLine > 0) {
                $added[] = [$tailLine-1, $tailPos+2];
            }
            // to the left beneath XorO and the trace stopped on index 1 or bigger and the line 
            // is above the  last line.
            if($game[$tailLine+1][$tailPos-2]  === $player 
