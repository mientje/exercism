...
function rebase(int $fromBase, array $inputDigits, int $toBase): array
{

    $result = [];

    if(validate($inputDigits, $fromBase, $toBase)) {
        // convert $digits to number in base 10
        $power = count($inputDigits)-1;
        $index = 0;
        $digits = $inputDigits;
        while($power >= 0) {
            $digits[$index] = ($fromBase ** $power) * $digits[$index];
            $power--;
            $index++;
        }
...
function validate(array $digits, int $fromBase, int $toBase) : int {
