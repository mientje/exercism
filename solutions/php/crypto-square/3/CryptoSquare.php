...
function crypto_square(string $plaintext): string
{
    ...
    return createSquare($splitText, $divider);

}

function createSquare($splitText, $divider) : string {
    $square = "";
    $lastRow = end( $splitText);
    $splitText[count($splitText)-1] .= str_repeat(' ', strlen($splitText[0]) - strlen($lastRow));
...
function defineRowsColumns(int $lengte) : int {
    ...
    return ($lengte - $rowDivider > $lengte - $colDivider ) ?  
        $rows : $columns; 
}