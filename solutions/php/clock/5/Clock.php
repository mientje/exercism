...
class Clock
{
    ...
    public function __construct(float $hours, int $minutes=0) {
        $this->hours = $hours;
        $this->minutes = $minutes;
        $this->timeString = $this->buildTimeString();
    }
    
    private function time(): void {
        $this->hours += floor($this->minutes/60);
        $this->hours = $this->calcTimeComponents($this->hours, self::DAY);
        $this->minutes = $this->calcTimeComponents($this->minutes, self::HOUR);
    }
    
    private function calcTimeComponents(float $comp, int $divider) : int {
         if($comp < 0) { 
