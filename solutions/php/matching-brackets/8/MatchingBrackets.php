...
function brackets_match(string $input): bool
{
    ...
    if(findMatchingBracket($input[0], [['}', null], [']', null ], [')', null ]])) { echo "returning false"; return false;}

    // string ends with an opening bracket
    if(findMatchingBracket($newInput[strlen($newInput)-1], [['{', null], ['[', null ], ['(', null ]])) { return false;}
...
function findMatchingBracket($char, $brackets) : string|null  {
  return match ($char) {
