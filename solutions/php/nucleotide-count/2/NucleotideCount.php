...
function nucleotideCount(string $input): array
{
   ...
   while(count($splitInput) > 0) {
		$last = $splitInput[count($splitInput)-1];
		$nucleotideChar = array_search($last, $splitInput); 
		if(is_int($nucleotideChar)) {
			$nucleotides = array_splice($splitInput, $nucleotideChar);
			$result[strtolower($last)] = count($nucleotides);
		}
	}
