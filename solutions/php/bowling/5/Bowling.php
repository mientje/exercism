...
class Game
{
    ...
    private function validateScore() : void {
        ...
        if($rollsThrown === self::MAX_THROWS) {
            ...
            if($firstLastThrow === self::MAX_POINTS) {
                throw new Exception("Two bonus rolls for a MAX_POINTS in the last frame must be rolled before the score can be calculated");
            }
            // spare in tenth frame - one more bonus roll
            else {
                if($firstLastThrow  + $secondLastThrow  === self::MAX_POINTS) {
                    throw new Exception("A bonus roll for a MAX_POINTS in the last frame must be rolled before the score can be calculated");
                }
            }
        }
        //FILL_BALLS
        ...
        if($firstLastThrow === self::MAX_POINTS 
            && $rollsThrown === self::MAX_THROWS + 1) {
            throw new Exception("One more bonus roll for a MAX_POINTS in the last frame must be rolled before the score can be calculated");
        }
        ...
        if($rollsThrown === self::MAX_THROWS + 2 
            && $this->thrown[$rollsThrown ] === self::MAX_POINTS) {
            throw new Exception("The bonus roll was a MAX_POINTS. One more bonus roll must be rolled before the score can be calculated");
        }         
    }
    ...
    private function validateRoll(int $pins) : void {
        ...
        if ($rollsThrown === self::MAX_THROWS + FILL_BALLs -1 
