...
function encode(string $input): string
{
    ...
    while(strlen($input) > 0) {
        preg_match("/$input[0]+/", $input, $matches);
        $repeatingLength = strlen($matches[0]);
        $output .= ($repeatingLength > 1) ? $repeatingLength . substr($input, 0, 1) : substr($input, 0, 1);
        $input = substr_replace($input, '', 0, $repeatingLength);
    }
    return $output;
}
