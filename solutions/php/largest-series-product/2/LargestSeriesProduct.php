...
class Series
{
    ...
    public function largestProduct(int $span): int
    {
        ...
        if($this->validation($span)) { return 1;}

        else {
            $index = 0;
            $largestProduct = 0;
            while($index < $this->lengte ) {
                $series = substr($this->input, $index, $span);
    ...
    private function validation(int $span) : mixed {

        if($span === 0) {
            return 1;            
        } 
        elseif($span > $this->lengte) {
