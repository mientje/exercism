...
class FlowerField
{
	...
	public function __construct(private array $garden)
   {
	...
	private function isItAFlower($rowIndex, $position ) : int { 
		if($position !== FlowerField::INVALID && $this->garden[$rowIndex][$position] === "*") { 
			$this->surroundedby += 1; 	
		} 
 		...
 		for($flowerIndex = 0; $flowerIndex < $this->rowLength ; $flowerIndex++) {
			if($row[$flowerIndex] === ' ') {
				...
				$this->findFlowers($rowIndex, $flowerIndex);													
 				if($above !== FlowerField::INVALID) { $this->findFlowers($above, $flowerIndex); };  
				if($beneath!== FlowerField::INVALID) { $this->findFlowers($beneath, $flowerIndex); }
