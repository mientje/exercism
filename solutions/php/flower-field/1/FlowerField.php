...
class FlowerField
{
	...
	public function __construct(array $garden)
   {
	...
	public function examineBoundary($rowIndex, $position ) : int {
				...
				$row = str_split($this->garden[$rowIndex]);
 				...
 				for($flowerIndex = 0; $flowerIndex < $this->rowLength ; $flowerIndex++) {
					...
					if($row[$flowerIndex] === ' ') {
						...
						$this->examineBoundary($rowIndex, $toTheRight);			
						$this->examineBoundary($rowIndex, $toTheLeft);		
										
 						if($above > -1) { 
 						 	$this->examineBoundary($above, $flowerIndex);
 						 	$this->examineBoundary($above, $toTheRight);
 						 	$this->examineBoundary($above, $toTheLeft);
 						} 
						if($beneath > -1) {
 						 	$this->examineBoundary($beneath, $flowerIndex);
 						 	$this->examineBoundary($beneath, $toTheRight);
 						 	$this->examineBoundary($beneath, $toTheLeft);
 						}
 						$row[$flowerIndex] = ($this->surroundedby === 0) ? ' ' : $this->surroundedby;   
 						$this->surroundedby = 0;
					}
				}
				$boundaryGarden[] = implode($row);
			}
	 		return $boundaryGarden;	
		}	
   }
}
