...
class FlowerField
{
	...
	public function __construct(private array $garden)
   {
	...
	private function isItAFlower($rowIndex, $position ) : int { 
		if($position !== self::INVALID && $this->garden[$rowIndex][$position] === "*") { 
			$this->surroundedby += 1; 	
		} 
	...
	private function buildResultRow($row, $rowIndex) : array {
 		...
 		for($flowerIndex = 0; $flowerIndex < $this->rowLength ; $flowerIndex++) {
			if($row[$flowerIndex] === ' ') {
				...
				$above =  ($rowIndex > 0) ? $rowIndex - 1 : self::INVALID;  
 				$beneath = ($rowIndex < $this->numRows - 1) ? $rowIndex + 1 : self::INVALID; 

				$this->findFlowers($rowIndex, $flowerIndex);													
 				if($above !== self::INVALID) { $this->findFlowers($above, $flowerIndex); };  
				if($beneath!== self::INVALID) { $this->findFlowers($beneath, $flowerIndex); }
