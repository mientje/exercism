...
{
    $validation = validate($amount, $coins);
    ...
    if(!$change[0]) {
        foreach($reversedCoins as $coin) {
            ...
            $calculatedChange = calculateChange($reversedCoins, 0, $amount, [], $amount);
            if($calculatedChange != [-1]) {
...
function validate($amount, $coins) {
...
function sortShortestArray($change) : array {
    if(count($change) === 0 
        || (count($change) === 1 && is_int($change[0]))) { 
        return $change;
    }
    $shortest = 0;
    $lengte = count($change);
    for($i = 1; $i < $lengte; $i++) {
        if(count($change[$i]) < count($change[$shortest])) {
            $shortest = $i;
        }
    }   
...
function calculateChange(array $coins, int $start, int $amount, array $change, int $originalAmount):array {
    ...
    if($changeAmount > $originalAmount || $changeAmount + $coins[$lengte] > $originalAmount) {
        array_pop($change);
