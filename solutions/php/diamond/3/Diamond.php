...
function diamond(string $letter): array
{
    ...
    for($index = 0, $lengte = count($alphabet)-1, $sidePadding = count($alphabet)-1; $index <= $lengte; $index++, $sidePadding-- ) {
        $curLetter = $alphabet[$index];
        $lineLength = ($lengte * 2) + 1;
        $centrePadding = $lineLength - ($sidePadding * 2) - 2; // 2 is two letters
        $diamondString = $curLetter . str_pad('', $centrePadding, ' ', STR_PAD_RIGHT) . $curLetter;
        $diamondLine = ($index === 0) ?
