...
function diamond(string $letter): array
{
    ...
    for($index = 0, $lengte = count($alphabet)-1, $sidePadding = $lengte; 
        $index <= $lengte; $index++, $sidePadding-- ) {
        $curLetter = $alphabet[$index];
        $lineLength = ($lengte * 2) + 1;
        $diamondCenter = $curLetter . str_pad('', $lineLength - ($sidePadding * 2) - 2, ' ', STR_PAD_RIGHT) . $curLetter;
        $diamondLine = ($index === 0) ?
            str_pad($curLetter, $lineLength, " ", STR_PAD_BOTH) :
            str_pad($diamondCenter, strlen($diamondCenter) + $sidePadding*2, ' ', STR_PAD_BOTH);
        $topHalf[] = $diamondLine;        
    }
