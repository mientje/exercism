...
function findWhich(int $year, int $month, string $weekday, string $which) {
	...
	if($which === 'last' ) {
		if($month === 2) {
			return ($date->modify("fourth " . $weekday)->format('m') != $month ) ?
				$date->modify("third " . $weekday) -> format('d') : 
				$date->modify("fourth " . $weekday) -> format('d');			       
		} else {
          return ($date->modify("fifth " . $weekday)->format('m') != $month ) ? 
				$date->modify("fourth " . $weekday) -> format('d') : 
				$date->modify("fifth " . $weekday) -> format('d');			       
		} 
	 }  	 
